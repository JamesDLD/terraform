resource "azurerm_template_deployment" "backup_vault" {
  ...
  template_body       = "${file("./modules/Create-AzureRmKeyVault/AzureRmKeyVault_template.json")}"
...
resource "azurerm_template_deployment" "backup_policies" {
  ...
  resource_group_name = "${var.rsv_resource_group_name}"
  template_body       = "${file("./modules/Create-AzureRmKeyVault/AzureRmKeyVaultPolicy_template.json")}"
  deployment_mode     = "Incremental"
