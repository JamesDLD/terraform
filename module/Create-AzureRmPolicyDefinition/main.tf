...
resource "azurerm_policy_definition" "policies" {
  ...
  policy_rule  = "${file("${path.module}/json_files/${lookup(var.policies[count.index], "suffix_name")}_policy_rule.json")}"
  parameters   = "${file("${path.module}/json_files/${lookup(var.policies[count.index], "suffix_name")}_parameters.json")}"
