resource "null_resource" "target_ids" {
  ...
  triggers = {
    source_vm_id     = var.bck_vms[count.index]["BackupPolicyName"] == "777" ? "" : "/subscriptions/${var.subscription_id}/resourceGroups/${element(var.bck_vms_resource_group_names, count.index)}/providers/Microsoft.Compute/virtualMachines/${element(var.bck_vms_names, count.index)}"
    backup_policy_id = var.bck_vms[count.index]["BackupPolicyName"] == "777" ? "" : "/subscriptions/${var.subscription_id}/resourceGroups/${var.bck_rsv_resource_group_name}/providers/Microsoft.RecoveryServices/vaults/${var.bck_rsv_name}/backupPolicies/${var.bck_vms[count.index]["BackupPolicyName"]}"
  }
}
